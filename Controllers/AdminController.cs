...
namespace BreakFastShop.Controllers
{
    public class AdminController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> UpsertMeal(Guid shopId, Guid? id, string name, decimal money, Guid? categoryId, string element, string optionsJson, bool isActive = true)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                return Json(new { ok = false, error = "Name required" });
